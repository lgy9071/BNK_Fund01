...
public interface FundRepository extends JpaRepository<Fund, String> {
        ...
        /** 내 투자성향 riskLevel 이하만 */
        @Query("""
                        select f from Fund f
                        where f.riskLevel <= :maxRisk
                          and exists (
                            select 1 from FundProduct fp
                            where fp.fund = f
                              and (:status is null or fp.status = :status)
                          ...
                          and (:keyword is null or :keyword = '' or lower(f.fundName) like lower(concat('%', :keyword, '%')))
                        """)
        Page<Fund> findAllHavingProductWithMaxRisk(
                        @Param("keyword") String keyword,
                        @Param("status") String status,
                        @Param("maxRisk") int maxRisk,
                        Pageable pageable);
 ...
 * - startRiskLevel 최소 위험등급 (투자성향 기반)
 * - endRiskLevel 최대 위험등급 (투자성향 기반)
 * - riskLevels 사용자가 선택한 위험등급 리스트 (null 가능)
 * - fundTypes 사용자가 선택한 펀드유형 리스트 (null 가능)
 * - regions 사용자가 선택한 투자지역 리스트 (null 가능)
 * - pageable 페이지네이션 정보
 * 
