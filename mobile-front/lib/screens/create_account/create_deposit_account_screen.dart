...
class CreateDepositAccountScreen extends StatefulWidget {
  ...
  @override
  State<CreateDepositAccountScreen> createState() => _CreateDepositAccountScreenState();
}
...
class _CreateDepositAccountScreenState extends State<CreateDepositAccountScreen> {
  ...
  Future<void> _loadUserProfile() async {
    ...
    try {
      final userService = widget.userService ?? UserService(); // 기본 UserService 사용
  ...
  void _validateForm() {
    ...
    setState(() {
      ...
      // PIN은 4자리 숫자만 필수
      _isFormValid = accountPin.length == 4 &&
          RegExp(r'^\d{4}$').hasMatch(accountPin);
    });
  }
  ...
  Future<void> _createAccount() async {
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
          ...
          if (mounted) {
            ...
            showDialog(
              ...
              builder: (BuildContext context) {
                return AlertDialog(
                  title: const Text('계좌 생성 완료'),
                  content: Column(
                    mainAxisSize: MainAxisSize.min,
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(responseData['message'] ?? '입출금 계좌가 성공적으로 생성되었습니다.'),
                      const SizedBox(height: 16),
                      Text('계좌번호: ${accountData['accountNumber']}'),
                      Text('계좌명: ${accountData['accountName']}'),
                      Text('잔액: ${accountData['balance']}원'),
                    ],
                  ),
                  actions: [
                    TextButton(
        ...
        throw Exception(errorData['message'] ?? '계좌 생성 실패: ${response.statusCode}');
      }
    } catch (e) {
      debugPrint('계좌 생성 실패: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('계좌 생성에 실패했습니다: $e')),
        );
      }
    } finally {
      if (mounted) {
  ...
  Future<void> _createAccount() async {
    ...
    try {
      ...
      final requestBody = {
        ...
        'accountPin': _accountPinController.text.trim(),
      };

      final uri = Uri.parse('https://api.example.com/accounts/deposit/create');
      debugPrint('[POST] $uri');
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('입출금 계좌가 성공적으로 생성되었습니다.'),
              backgroundColor: primaryColor,
            ),
          );
          Navigator.of(context).pop(); // 이전 화면으로 돌아가기
        ...
        throw Exception('계좌 생성 실패: ${response.statusCode}');
      }
    } catch (e) {
      debugPrint('계좌 생성 실패: $e');
      if (mounted) {
